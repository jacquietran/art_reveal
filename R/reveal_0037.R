...
# General params
iteration_id <- "reveal_0037"
initial_seed <- 721137
n_tiles <- 9
bg_colour <- "#242331"
line_colour <- "#DDCA7D"

grain_colours <- c("#533E2D", "#A27035", "#B88B4A", "#519872", "#DC758F")
...
grain_data <- mimic_grain2(
  ...
  grain = 800, n_lines = 100, colours = grain_colours)
...
img_left <- img |> magick::image_crop("3300x9000")
img_right <- img |> magick::image_crop("5700x9000+3300")

img_left_invert <- img_left |>
  magick::image_negate()

img_recombined <- magick::image_append(c(img_left_invert, img_right))
