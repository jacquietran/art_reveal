...
# General params
iteration_id <- "reveal_0055"
initial_seed <- 323455
bg_colour <- "#F9F9F9"
line_colour <- "#7FC8F8"
colour_opts <- c("#5AA9E6", "#FFE45E", "#FF6392", bg_colour)

# Create data ------------------------------------------------------------------

blobs_combined <- blobs_and_tiles(
  arrangement = "20x20", seed_vec = initial_seed, bg = bg_colour,
  colours = colour_opts, groups_min = 12, groups_max = 60)
...
p <- ggplot2::ggplot() +
  ggforce::geom_diagonal_wide(
      ...
      ggplot2::aes(x = x, y = y, group = grouping_var, size = group_size * 0.6,
                   ...
                   fill = group_fill),
      strength = 10, colour = line_colour, alpha = 1) +
