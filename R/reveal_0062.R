...
blobs_combined <- blobs_and_tiles(
  ...
  colours = colour_opts, groups_min = 30, groups_max = 100)
...
p <- ggplot2::ggplot() +
  ggforce::geom_diagonal_wide(
      data = blobs_combined,
      ggplot2::aes(x = x, y = y, group = grouping_var, size = group_size * 0.6,
                   linetype = group_linetype,
                   fill = group_fill),
      strength = 15, colour = line_colour, alpha = 1) +
