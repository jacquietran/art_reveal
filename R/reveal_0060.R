...
# General params
iteration_id <- "reveal_0060"
initial_seed <- 323460
bg_colour <- "#D8CBC7"
line_colour <- "#19231A"
colour_opts <- c("#CC3F0C", "#9A6D38", "#33673B", bg_colour)

# Create data ------------------------------------------------------------------

blobs_combined <- blobs_and_tiles(
  arrangement = "5x5", seed_vec = initial_seed, bg = bg_colour,
...
p <- ggplot2::ggplot() +
  ggforce::geom_diagonal_wide(
      data = blobs_combined,
      ggplot2::aes(x = x, y = y, group = grouping_var, size = group_size * 0.8,
                   #linetype = group_linetype,
                   fill = group_fill),
      strength = 0, colour = line_colour, alpha = 1,
      radius = ggplot2::unit(2, 'mm')) +
